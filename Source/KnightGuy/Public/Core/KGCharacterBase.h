...
#include "Abilities/KGAttributeSet.h"
#include "Items/KGTypes.h"
...
UCLASS()
class KNIGHTGUY_API AKGCharacterBase : public ACharacter, public IAbilitySystemInterface
{
    GENERATED_BODY()
  ...
  public:
	...
	*/
	...
	bool ActivateAbilitiesWithItemSlot(FKGItemSlot ItemSlot, bool bAllowRemoteActivation = true);

	/** Returns a list of active abilities bound to the item slot. This only returns if the ability is currently running */
	UFUNCTION(BlueprintCallable, Category = "Abilities")
	void GetActiveAbilitiesWithItemSlot(FKGItemSlot ItemSlot, TArray<UKGGameplayAbility*>& ActiveAbilities);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Abilities")
	void GetActiveAbilitiesWithTags(FGameplayTagContainer AbilityTags, TArray<UKGGameplayAbility*>& ActiveAbilities);
  ...
  protected:
	...
	*/
	...
	void OnDamaged(float DamageAmount, const FHitResult& HitInfo, const struct FGameplayTagContainer& DamageTags, AKGCharacterBase* InstigatorCharacter, AActor* DamageCauser);
	...
	*/
	...
	void OnItemSlotChanged(FKGItemSlot ItemSlot, UKGItem* Item);
	...
	void FillSlottedAbilitySpecs(TMap<FKGItemSlot, FGameplayAbilitySpec>& SlottedAbilitySpecs);
	...
	// Called from KGAttributeSet, these call BP events above
	virtual void HandleDamage(float DamageAmount, const FHitResult& HitInfo, const struct FGameplayTagContainer& DamageTags, AKGCharacterBase* InstigatorCharacter, AActor* DamageCauser);
  ...
  protected:
    ...
    TArray<TSubclassOf<UKGGameplayAbility>> GameplayAbilities;

    /** Map of item slot to gameplay ability class, these are bound before any abilities added by the inventory */
    //UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Abilities)
    TMap<FKGItemSlot, TSubclassOf<UKGGameplayAbility>> DefaultSlottedAbilities;
	...
	TMap<FKGItemSlot, FGameplayAbilitySpecHandle> SlottedAbilities;
  ...
  private:
    ...
    UPROPERTY()
    UKGAbilitySystemComponent * AbilitySystemComponent;

    /** List of attributes modified by the ability system */
    UPROPERTY()
    UKGAttributeSet * AttributeSet;


	/** Cached pointer to the inventory source for this character, can be null */
	UPROPERTY()
	TScriptInterface<IKGInventoryInterface> InventorySource;
