...
#include "Inventory/KGInventoryInterface.h"
#include "KGPlayerControllerBase.generated.h"


/** Base class for PlayerController, should be blueprinted */
UCLASS()
class KNIGHTGUY_API AKGPlayerControllerBase : public APlayerController, public IKGInventoryInterface
{
	GENERATED_BODY()
...
public:
	...
	TMap<UKGItem*, FKGItemData> InventoryData;

	/** Map of slot, from type/num to item, initialized from ItemSlotsPerType on KGGameInstanceBase */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = Inventory)
	TMap<FKGItemSlot, UKGItem*> SlottedItems;

public:
    // Constructor and overrides
    AKGPlayerControllerBase() {}
    virtual void BeginPlay() override;


    /** Called after the inventory was changed and we notified all delegates */
    UFUNCTION(BlueprintImplementableEvent, Category = Inventory)
    void InventoryItemChanged(bool bAdded, UKGItem* Item);

    /** Called after an item was equipped and we notified all delegates */
    UFUNCTION(BlueprintImplementableEvent, Category = Inventory)
    void SlottedItemChanged(FKGItemSlot ItemSlot, UKGItem* Item);
	...
	// Implement IKGInventoryInterface
	virtual const TMap<UKGItem*, FKGItemData>& GetInventoryDataMap() const override
	{
		return InventoryData;
	}
	virtual const TMap<FKGItemSlot, UKGItem*>& GetSlottedItemMap() const override
	{
		return SlottedItems;
	}
...
public:
  ...
  protected:
	...
	bool FillEmptySlotWithItem(UKGItem* NewItem);

	/** Calls the inventory update callbacks */
	void NotifyInventoryItemChanged(bool bAdded, UKGItem* Item);
	void NotifySlottedItemChanged(FKGItemSlot ItemSlot, UKGItem* Item);
	void NotifyInventoryLoaded();

	/** Called when a global save game as been loaded */
	void HandleSaveGameLoaded(UKGSaveGame* NewSaveGame);
};
